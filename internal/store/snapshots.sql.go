...
WITH s AS (
  ...
  SELECT month, movie_id, closed_at, popularity, title, release_date, overview, poster_path, backdrop_path, imdb_url, cinemagia_url, solo_friends, couple, streaming, arr, key_value FROM keyed
  WHERE (
    $6::float8 IS NULL OR (
